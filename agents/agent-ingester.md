...
tags:
  ...
  - area/governance
  - status/active
purpose: "Atomize one freeform inbox/ item into one-idea-per-file typed notes (prose) and proposed migration rows (records), carrying provenance, then clear the inbox entry — proposing via a human-gated PR, never writing the DB"
profile: standard
inputs: ["one inbox/ item (a phone dump, pasted text, voice-to-text, a quick thought)", "the conventions + the decomposition recipe"]
outputs: ["atomic typed notes wired into the right moc-<domain> via related, with provenance", "migration file(s) proposing record rows", "a cleared inbox entry", "an inbox/attention/ note for any ambiguous routing"]
...
([[rule-synapse-fail-loudly]], [[rule-no-unprompted-actions]]). Don't restate a fact that already lives
somewhere — link it ([[rule-synapse-single-source-of-truth]]).
