...
//
//   node _meta/tools/augment.mjs <agent> [<target>] [--profile P] --task "<text>" [--k N] [--no-semantic]
//
...
// modifies it; it shells out and parses the `<!-- <id> (<type>) -->` markers it already emits.
//
// Phase 2 (semantic augment): embed the query (the user's task) via the SAME local Ollama path
// as gen-embeddings.mjs, open `note_vectors` READ-ONLY, cosine-rank notes NOT already in the closure, take
// the top-K under a token budget, and APPEND a clearly-labeled "## Semantically related (not yet linked)"
// section. Results are additive, labeled, never authoritative ([[rule-semantic-suggests-links-decide]]).
//
// Graceful degradation: --no-semantic, Ollama unreachable, or an empty/absent note_vectors → print the
// deterministic briefing plus a "(semantic augment skipped: <reason>)" note. NEVER crash; never block the
// deterministic spine ([[rule-semantic-suggests-links-decide]], [[rule-synapse-fail-loudly]]).
//
...
const MIN_SIM = Number(process.env.SYNAPSE_MIN_SIM ?? 0.45);  // floor: skip weak/irrelevant hits (tune per embed model)
...
// --selftest: offline checks of the ranking/RRF/budget logic (no Ollama, no DB).
...
if (!agent) {
  console.error('usage: augment <agent> [<target>] [--profile P] --task "<text>" [--k N] [--no-semantic]');
  process.exit(2);
}

// ===================================================================== Phase 1: deterministic render (child)
const renderArgs = [RENDER, agent];
if (target) renderArgs.push(target);
...
process.stderr.write(r.stderr || ""); // pass render's [render …] log line through to stderr
...
async function augment() {
  if (!existsSync(DB_PATH)) return skip("db/synapse.db not found");
  ...
  try {
    ...
    const has = db
      ...
      .get();
    if (!has) { db.close(); return skip("note_vectors absent — run gen-embeddings.mjs"); }
    rows = db.prepare(`SELECT id, model, dim, vec FROM note_vectors`).all();
    db.close();
  } catch (e) {
    return skip(`could not read note_vectors (${e.message})`);
  }
  if (!rows.length) return skip("note_vectors empty — run gen-embeddings.mjs");
  ...
  process.stderr.write(
    `[augment] semantic: ${shown}/${ranked.length} out-of-closure hit(s) appended ` +
