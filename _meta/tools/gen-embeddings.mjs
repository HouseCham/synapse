#!/usr/bin/env node
// gen-embeddings.mjs — embed every note body via local Ollama → a generated `note_vectors` table.
//
//   node _meta/tools/gen-embeddings.mjs            # incremental: re-embed only changed/new notes
//   node _meta/tools/gen-embeddings.mjs --all       # force a full re-embed of every note
//   node _meta/tools/gen-embeddings.mjs --selftest   # offline math check (no Ollama, no DB writes)
//
// This is a GENERATED, derived projection of the vault — like gen-index.mjs ([[rule-derived-views-are-
// generated]]). It OWNS its table (CREATE TABLE IF NOT EXISTS), so it needs no migration; the table is
// never canonical and never hand-edited. Markdown is the source of truth; `note_vectors` is a rebuildable
// cache of float embeddings used by augment.mjs for semantic recall ([[doc-semantic-recall]]).
//
// Embeddings come from the SAME local Ollama that runs the agents (over Tailscale, no API key) —
// see [[tool-ollama-embeddings]]. The base URL + model are resolved from env / opencode.json, NEVER
// hardcoded. If Ollama is unreachable or the model is missing, this exits with a clear message and
// leaves any existing vectors intact ([[rule-semantic-suggests-links-decide]]).
//
// v1 storage: plain Float32Array BLOB; cosine is computed in JS by augment.mjs (brute force is ample at
// personal-vault scale). `sqlite-vec` is the documented drop-in scale-up swap when the vault grows large.
//
...
// Resolve the Ollama base URL: env SYNAPSE_OLLAMA_URL, else opencode.json provider baseURL (strip /v1),
// else localhost. NEVER hardcode a Tailnet hostname or key.
export function resolveOllamaBase() {
  if (process.env.SYNAPSE_OLLAMA_URL) return process.env.SYNAPSE_OLLAMA_URL.replace(/\/+$/, "");
  try {
    const cfgPath = join(homedir(), ".config", "opencode", "opencode.json");
    if (existsSync(cfgPath)) {
      const cfg = JSON.parse(readFileSync(cfgPath, "utf8"));
      const base = cfg?.provider?.ollama?.options?.baseURL;
      if (base) return String(base).replace(/\/+$/, "").replace(/\/v1$/, "");
    }
  } catch {
...
export function resolveEmbedModel() {
  return process.env.SYNAPSE_EMBED_MODEL || "mxbai-embed-large";
}
...
// how to degrade). `fetch` is built into Node 25 — no npm dep.
...
// raw → frontmatter/body split + type/title (mirrors gen-index.mjs / render.mjs).
...
async function main() {
  if (!existsSync(DB_PATH)) {
    console.error(`[gen-embeddings] db not found at ${DB_PATH} — run apply-migrations.mjs first.`);
    process.exit(1);
  }
  ...
  const SKIP = new Set(
    JSON.parse(readFileSync(join(HERE, "context.manifest.json"), "utf8")).skipDirs || []
  );

  // walk (mirror gen-index/render: skip dotdirs + manifest.skipDirs)
  function walk(dir) {
    const out = [];
  ...
  if (!todo.length) {
    ...
    console.log(
      `[gen-embeddings] up to date — embedded 0, skipped ${skipped}, removed ${removed}` +
  ...
  for (const id of todo) {
    ...
    try {
      ...
      console.error(`[gen-embeddings] FAILED at note "${id}": ${e.message}`);
      console.error(`[gen-embeddings] existing vectors left intact. Embedded ${embedded} before failure.`);
      db.close();
      process.exit(1);
    }
    dim = vec.length;
  ...
  console.log(
    `[gen-embeddings] embedded ${embedded}, skipped ${skipped}, removed ${removed}` +
